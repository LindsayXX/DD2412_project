...
class RCN(tf.keras.Model):
    ...
    def boxcar(self, t, k=10):
        ...
        for n in range(t.shape[0]):
            ...
            '''
            X = tf.range(start=0, limit=V.shape[1])
            Y = tf.range(start=0, limit=V.shape[2])
            X, Y = tf.meshgrid(X, Y)
            V_x = tf.constant(1) / (tf.constant(1) + tf.math.exp(-k * (X - t[n, 0] + t[n, 2]/tf.constant(2)))) - tf.constant(1) / (tf.constant(1) + tf.math.exp(-k * (X - t[n, 0] - t[n, 2]/tf.constant(2))))
            V_y = tf.constant(1) / (tf.constant(1) + tf.math.exp(-k * (Y - t[n, 1] + t[n, 2]/tf.constant(2)))) - tf.constant(1) / (tf.constant(1) + tf.math.exp(-k * (Y - t[n, 1] - t[n, 2]/tf.constant(2))))
